...
export async function GET(){
 try{
  const sql=db();
  const docs=await sql`SELECT id,repository_path,frontmatter,updated_at FROM galgo.documents WHERE repository_path=${SOURCE} LIMIT 1`;
  ...
  const rows=await sql`SELECT sn.label AS source,sn.node_type AS source_type,sn.properties AS source_properties,
    ...
    WHERE d.repository_path=${SOURCE} AND e.edge_type <> 'MAPS_NODE'
