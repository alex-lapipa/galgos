...
import {corpusMetadata} from './corpus-metadata';
...
function parse(file:string,raw:string):CorpusDoc{
 const parsed=matter(raw);const native=parsed.data as Record<string,unknown>;const sidecar=corpusMetadata[file]??{};const data={...sidecar,...native};const policy=resolveCorpusPolicy(file,data);
