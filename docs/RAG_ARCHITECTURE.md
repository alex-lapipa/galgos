...
- `galgo.chunks`: section-aware text chunks, full-text index and 1536d vectors.
...
## Retrieval
`galgo.hybrid_search` uses reciprocal-rank fusion over HNSW cosine similarity and PostgreSQL full-text search. The default weighting is 70% semantic / 30% lexical. `app/api/ask` then sends only retrieved passages to the generation model and returns path/line citations.

## Models
- Embeddings: `openai/text-embedding-3-small` through Vercel AI Gateway/OIDC, 1536 dimensions.
- Generation: `openai/gpt-5.6-terra` through Vercel AI Gateway/OIDC; override with `RAG_GENERATION_MODEL`.
