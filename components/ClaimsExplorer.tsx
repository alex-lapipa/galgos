...
export function ClaimsExplorer({claims,sourceLinks}:{claims:ClaimRecord[];sourceLinks:Record<string,string>}){
 ...
 const sourceLink=(source:string)=>sourceLinks[source]||`/archive?search=${encodeURIComponent(source)}`;
 return <main className={styles.page}><div className={styles.inner}>
  ...
  <section className={styles.ledger}>{filtered.map(claim=><article key={claim.claimId} className={styles.claim}><div className={styles.claimHead}><span>{claim.claimId}</span><div><b className={styles.evidence}>{claim.evidenceLevel}</b><b className={styles.dimension}>{claim.dimension}</b></div></div><h2>{claim.statement}</h2><p className={styles.note}>{claim.note}</p><dl><div><dt>Review status</dt><dd>{claim.reviewStatus.replaceAll('_',' ')}</dd></div><div><dt>Supporting / contextual sources</dt><dd>{claim.supportingSources.length?claim.supportingSources.map(source=><Link key={source} href={sourceLink(source)}>{source}</Link>):<span>None recorded</span>}</dd></div><div><dt>Contradicting / limiting sources</dt><dd>{claim.limitingSources.length?claim.limitingSources.map(source=><Link key={source} href={sourceLink(source)}>{source}</Link>):<span>None recorded</span>}</dd></div></dl><div className={styles.actions}><Link href={`/ask?q=${encodeURIComponent(`Evaluate this claim from the archive: ${claim.statement}`)}`}>Ask Archive about this claim →</Link><Link href="/archive/galgo-espanol-claim-ledger">Open canonical ledger →</Link></div></article>)}</section>
